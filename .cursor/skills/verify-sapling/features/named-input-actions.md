...
- **Space confirm.** Run `control-sapling key space`. Arena HUD appears.
- **Reset.** Run `control-sapling key escape` to title.
...
- **Quit vs back.** From arena, Run `control-sapling key escape` (title, PID alive). From title, Run `control-sapling key escape` (PID exits).

## Gotchas

- Sokol key names in the helper are X11/mac keysyms (`space`, `Return`, `Escape`, `w`, `Left`). Manifest names are `SPACE`, `ENTER`, `ESCAPE`, `W`, `LEFT`.
